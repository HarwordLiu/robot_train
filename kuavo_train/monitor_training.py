...
class TrainingMonitor:
    ...
    def get_latest_metrics(self) -> Dict:
        ...
        if self.event_acc:
            ...
            validation_tags = [
                tag for tag in all_tags if tag.startswith('validation/')]
            ...
            if validation_tags:
                ...
                for tag in validation_tags:
                    ...
                    if val_data:
                        task_name = tag.replace(
                            'validation/', '').replace('_loss', '')
    ...
    def get_checkpoint_info(self) -> Dict:
        ...
        if epoch_dirs:
            info['saved_epochs'] = [
                int(d.name.replace('epoch', '')) for d in epoch_dirs]
    ...
    def get_training_status(self, metrics: Dict) -> Dict:
        ...
        if len(loss_history) >= 5:
            ...
            loss_std = sum((x - sum(recent_losses)/len(recent_losses))
                           ** 2 for x in recent_losses) / len(recent_losses)
            ...
            if 'epoch_duration' in metrics:
                ...
                if estimated_total_time > 24 * 60:  # 超过24小时
                    status['suggestions'].append(
                        f'💡 预计总训练时间: {estimated_total_time/60:.1f}小时，建议调整batch size或减少epoch')
    ...
    def display_metrics(self, clear_screen: bool = True):
        ...
        if 'loss' in metrics:
            ...
            if len(loss_history) >= 2:
                ...
                loss_change_pct = (loss_change / prev_loss) * \
                    100 if prev_loss != 0 else 0

                change_symbol = "📉" if loss_change < 0 else "📈"
                print(
                    f"训练Loss: {metrics['loss']:.6f} {change_symbol} ({loss_change_pct:+.2f}%)")
            else:
        ...
        if 'epoch_duration' in metrics:
            ...
            if len(loss_history) >= 2:
                ...
                estimated_remaining_time = remaining_epochs * \
                    metrics['epoch_duration']

                if remaining_epochs > 0:
                    print(
                        f"预计剩余时间: {estimated_remaining_time:.1f}分钟 ({estimated_remaining_time/60:.1f}小时)")
        ...
        print("-" * 80)
        print(
            f"最佳模型: {'✅ 已保存' if checkpoint_info['best_exists'] else '❌ 未保存'}")
    ...
    def plot_metrics(self):
        ...
        # 3. Epoch耗时
        duration_data = self.metrics_history.get(
            'train/epoch_duration_minutes', [])
        if duration_data:
            ...
            if validation_tags:
                for tag in validation_tags:
                    ...
                    if val_data:
                        ...
                        axes[1, 1].plot(
                            steps, values, linewidth=2, label=task_name)
