...
def main():
    ...
    advanced_available = results.get(
        'rich', False) and results.get('matplotlib', False)
    gpu_monitoring = results.get('GPUtil', False)

    print(
        f"基础监控器:   {'✅ 可用' if basic_available else '❌ 不可用（需要安装tensorboard）'}")
    print(
        f"高级监控器:   {'✅ 可用' if advanced_available else '⚠️  部分功能不可用（需要安装rich和matplotlib）'}")
