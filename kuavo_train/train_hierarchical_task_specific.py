...
def load_task_dataset(task_id: int, cfg: DictConfig, policy_cfg, image_transforms) -> Tuple[Optional[LeRobotDataset], Optional[LeRobotDatasetMetadata]]:
    ...
    try:
        ...
        dataset_metadata = LeRobotDatasetMetadata(task_repoid, root=task_data_path)

        # 构建delta timestamps
        delta_timestamps = build_delta_timestamps(dataset_metadata, policy_cfg)

        # 应用episode限制
        max_episodes = cfg.task_specific_training.memory_management.get('max_episodes_per_task', 300)
        episodes_to_use = list(range(min(max_episodes, dataset_metadata.info["total_episodes"])))
...
def run_task_specific_curriculum_stage(policy, stage_config: Dict[str, Any], dataloader: DataLoader,
                                     ...
                                     optimizer, lr_scheduler, scaler, amp_enabled: bool,
    ...
    for epoch in range(stage_epochs):
        epoch_bar = tqdm(dataloader, desc=f"课程阶段 {stage_name} Epoch {epoch+1}/{stage_epochs}")
        epoch_loss = 0.0
        ...
        for batch in epoch_bar:
            ...
            with make_autocast(amp_enabled):
                loss, hierarchical_info = policy.forward(batch, curriculum_info=curriculum_info)
            ...
            if stage_steps % cfg.training.log_freq == 0:
                writer.add_scalar(f"curriculum/{stage_name}/loss", scaled_loss.item(), current_step + stage_steps)

                # 记录任务特定信息
                if isinstance(hierarchical_info, dict):
                    for key, value in hierarchical_info.items():
                        if isinstance(value, (int, float)):
                            writer.add_scalar(f"curriculum/{stage_name}/{key}", value, current_step + stage_steps)
    ...
    avg_stage_loss = total_loss / stage_steps if stage_steps > 0 else float('inf')
...
def save_task_specific_checkpoint(policy, optimizer, lr_scheduler, scaler, steps, epoch, best_loss,
    ...
    torch.save(checkpoint, output_directory / "task_specific_learning_state.pth")
...
def main(cfg: DictConfig):
    ...
    # 检测可用任务数据
    task_config = cfg.get('task_specific_training', {})
    available_tasks = task_config.get('data_config', {}).get('task_directories', {})
    ...
    first_task_dataset, first_task_metadata = load_task_dataset(first_task_id, cfg, policy_cfg, image_transforms)
    ...
    scaler = torch.amp.GradScaler(device=device.type, enabled=amp_enabled) if hasattr(torch, "amp") else torch.cuda.amp.GradScaler(enabled=amp_enabled)
    ...
    if use_curriculum:
        ...
        for stage_name, stage_config in curriculum_stages.items():
            current_step = run_task_specific_curriculum_stage(
                ...
                optimizer, lr_scheduler, scaler, amp_enabled, current_step
            )
        ...
        for batch in epoch_bar:
            ...
            if steps % cfg.training.log_freq == 0:
                ...
                if isinstance(hierarchical_info, dict):
                    for key, value in hierarchical_info.items():
                        if isinstance(value, (int, float)):
                            writer.add_scalar(f"task_specific/{key}", value, steps)
                        ...
                        if isinstance(stats, dict):
                            for stat_name, stat_value in stats.items():
                                if isinstance(stat_value, (int, float)):
                                    writer.add_scalar(f"performance/{layer_name}/{stat_name}", stat_value, steps)
