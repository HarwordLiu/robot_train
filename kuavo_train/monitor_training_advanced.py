...
class AdvancedTrainingMonitor:
    ...
    def _init_event_accumulator(self):
        ...
        if tb_files:
            try:
                ...
                if self.console:
                    self.console.print(f"[yellow]⚠️  加载TensorBoard事件失败: {e}[/yellow]")
        ...
        try:
            ...
            for tag in all_tags:
                try:
                    ...
                    self.metrics_cache[tag] = [(e.step, e.value, e.wall_time) for e in events]
                except:
                    pass
        except:
            pass
    ...
    def _calculate_health_score(self):
        ...
        if len(loss_data) >= 10:
            ...
            # 检查是否震荡过大
            loss_std = (sum((x - sum(recent_losses)/len(recent_losses))**2 for x in recent_losses) / len(recent_losses)) ** 0.5
        ...
        if self.monitor_gpu and self.gpu_history:
            recent_gpu = self.gpu_history[-10:] if len(self.gpu_history) >= 10 else self.gpu_history
    ...
    def get_training_summary(self) -> Dict:
        ...
        # 获取验证指标
        val_metrics = {k: v for k, v in self.metrics_cache.items() if k.startswith('validation/')}
        if val_metrics:
            summary['validation'] = {
    ...
    def render_rich_dashboard(self) -> Layout:
        ...
        layout["status"].update(Panel(status_text, title="训练状态", border_style=health_color))
        ...
        layout["resources"].update(Panel(resources_text, title="资源使用", border_style="blue"))
        ...
        layout["checkpoints"].update(Panel(ckpt_text, title="Checkpoint", border_style="magenta"))
    ...
    def create_matplotlib_dashboard(self):
        ...
        fig.suptitle(f'训练监控 - {self.run_dir.name}', fontsize=16, fontweight='bold')
        ...
        def update_plot(frame):
            ...
            if loss_data:
                ...
                ax_loss.plot(steps, values, 'b-', linewidth=2, label='Training Loss')
            ...
            # 3. Epoch耗时
            duration_data = self.metrics_cache.get('train/epoch_duration_minutes', [])
            if duration_data:
            ...
            if self.gpu_history:
                for gpu_idx in range(len(self.gpu_history[0]['utilization'])):
                    utils = [info['utilization'][gpu_idx] for info in self.gpu_history]
            ...
            # 5. 健康度评分
            ax_health.bar(['Health Score'], [self.health_score], color='green' if self.health_score >= 80 else 'orange')
            ax_health.set_ylim(0, 100)
            ax_health.set_title('训练健康度')
            ax_health.text(0, self.health_score + 2, f'{self.health_score}/100', ha='center', fontweight='bold')
            ...
            if val_metrics:
                for tag, data in val_metrics.items():
                    if data:
                        ...
                        ax_validation.plot(steps, values, linewidth=2, label=label, marker='o')
                ...
                ax_validation.text(0.5, 0.5, '暂无验证数据', ha='center', va='center')
